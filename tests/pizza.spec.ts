...
async function mockPizzaApi(page: Page) {
  let users: User[] = [
    ...
    { id: '2', name: 'Kai Chen', email: 'd@jwt.com', password: 'diner', roles: [{ role: Role.Diner }] },
    { id: '4', name: 'Franchisee', email: 'f@jwt.com', password: 'franchisee', roles: [{ role: Role.Diner }] },
  ];
  ...
  await page.route(/\/api\/franchise\/\d+/, async (route) => {
    ...
    if (method === 'GET') { 
        ...
        const userFranchises = franchises.filter(f => f.admins.some(a => a.id === idParam));
  ...
  await page.route(/\/api\/franchise\/\d+\/store$/, async (route) => {
    if (route.request().method() === 'POST') {
      ...
      if (franchise) {
        const newStore = { id: Math.floor(Math.random() * 1000), name: req.name, totalRevenue: 0 };
  ...
  await page.route(/\/api\/order$/, async (route) => {
    if (route.request().method() === 'POST') {
      ...
      await route.fulfill({ json: { dinerId: loggedInUser?.id, orders: orders, page: 1 } });
    }
  });
...
test('franchisee dashboard', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'Papi\'s 0 ₿ Close' }).getByRole('button').click();
  await page.getByRole('button', { name: 'Close' }).click();
