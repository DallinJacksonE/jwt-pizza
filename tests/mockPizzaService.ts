...
export async function mockPizzaApi(page: Page) {
	let users: User[] = [
		{
			id: "1",
			name: "Admin",
    ...
    await page.route(/\/api\/user\/(.+)/, async (route) => {
        ...
        if (method === 'PUT') {
            ...
            if (userIndex !== -1) {
                const updatedUser = { ...users[userIndex], ...route.request().postDataJSON() };
                users[userIndex] = updatedUser;
                await route.fulfill({ json: { user: updatedUser, token: "mock-jwt-token" } });
            } else {
