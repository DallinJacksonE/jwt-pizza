...
export default function DinerDashboard(props: Props) {
    ...
    (async () => {
      if (user?.id) {
        try {
          ...
          // This catch block prevents a failed network request from crashing the component.
        }
      }
    })();
    // Only re-run this effect if the user's ID changes (i.e., a different user logs in).
    // This prevents re-fetching orders every time the user's name/email is updated.
  }, [user?.id]);
  ...
  function handleOpenEditModal() {
    // Pre-fill the modal's state with the current user's data.
    setEditName(user.name || "");
    setEditEmail(user.email || "");
    setEditPassword(""); // Always clear password field for security.
    HSOverlay.open(document.getElementById("hs-jwt-modal")!);
  }

  async function updateUser() {
    try {
      const userFromServer = await pizzaService.updateUser({
        id: user.id,
        name: editName,
        email: editEmail,
      ...
      // Log the error without crashing the component.
